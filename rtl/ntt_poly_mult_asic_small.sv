...
module ntt_poly_mult_asic_small #(
    ...
    output logic                  a_input_mem_we,
    output logic [ADDR_WIDTH-1:0] a_input_mem_waddr,
    output logic [WIDTH-1:0]      a_input_mem_wdata,
    output logic                  b_input_mem_we,
    output logic [ADDR_WIDTH-1:0] b_input_mem_waddr,
    output logic [WIDTH-1:0]      b_input_mem_wdata,
    output logic [ADDR_WIDTH-1:0] input_mem_read_addr,
    input  logic [WIDTH-1:0]      a_mem_read_data,
    input  logic [WIDTH-1:0]      b_mem_read_data,

    output logic                  a_ntt_mem_we,
    output logic [ADDR_WIDTH-1:0] a_ntt_mem_addr,
    output logic [WIDTH-1:0]      a_ntt_mem_wdata,
    input  logic [WIDTH-1:0]      a_ntt_mem_rdata,
    output logic                  b_ntt_mem_we,
    output logic [ADDR_WIDTH-1:0] b_ntt_mem_addr,
    output logic [WIDTH-1:0]      b_ntt_mem_wdata,
    input  logic [WIDTH-1:0]      b_ntt_mem_rdata,
    output logic                  c_ntt_mem_we,
    output logic [ADDR_WIDTH-1:0] c_ntt_mem_addr,
    output logic [WIDTH-1:0]      c_ntt_mem_wdata,
    input  logic [WIDTH-1:0]      c_ntt_mem_rdata
);
  ...
  assign input_mem_read_addr =
      (input_mem_read_active && (load_index < N)) ? load_index[ADDR_WIDTH-1:0]
                                                  : debug_read_addr;
  assign a_input_mem_we = load_coeff && (state == IDLE) && !load_sel;
  assign a_input_mem_waddr = load_addr;
  assign a_input_mem_wdata = load_data;
  assign b_input_mem_we = load_coeff && (state == IDLE) && load_sel;
  assign b_input_mem_waddr = load_addr;
  assign b_input_mem_wdata = load_data;
  ...
  assign a_ntt_mem_we = a_ntt_we_a;
  assign a_ntt_mem_addr = a_ntt_we_a ? a_ntt_wr_addr : a_ntt_addr_b;
  assign a_ntt_mem_wdata = a_ntt_wr_data;
  assign a_ntt_dout_b = a_ntt_mem_rdata;

  assign b_ntt_mem_we = b_ntt_we_a;
  assign b_ntt_mem_addr = b_ntt_we_a ? b_ntt_wr_addr : b_ntt_addr_b;
  assign b_ntt_mem_wdata = b_ntt_wr_data;
  assign b_ntt_dout_b = b_ntt_mem_rdata;

  assign c_ntt_mem_we = c_ntt_we_a;
  assign c_ntt_mem_addr = c_ntt_we_a ? c_ntt_wr_addr : c_ntt_addr_b;
  assign c_ntt_mem_wdata = c_ntt_wr_data;
  assign c_ntt_dout_b = c_ntt_mem_rdata;
  ...
  always_ff @(posedge clk) begin
    ...
    end else if (state == POINTWISE) begin
  ...
  assign a_ntt_we_a = (state == READ_A) && read_pending;
  assign a_ntt_wr_addr = read_index[ADDR_WIDTH-1:0] - ADDR_WIDTH'(1);
  assign a_ntt_wr_data = fwd_read_data;
  assign a_ntt_addr_b = (state == POINTWISE && point_index < N)
                        ? point_index[ADDR_WIDTH-1:0] : '0;

  assign b_ntt_we_a = (state == READ_B) && read_pending;
  assign b_ntt_wr_addr = read_index[ADDR_WIDTH-1:0] - ADDR_WIDTH'(1);
  assign b_ntt_wr_data = fwd_read_data;
  assign b_ntt_addr_b = (state == POINTWISE && point_index < N)
                        ? point_index[ADDR_WIDTH-1:0] : '0;

  assign mul_a = a_ntt_dout_b;
  assign mul_b = b_ntt_dout_b;

  assign inv_start = (state == RUN_INV) && !inv_started;
  assign c_ntt_we_a = (MULT_PIPELINE == 0) ? bram_data_valid
  ...
  assign c_ntt_addr_b = (state == LOAD_INV && load_index < N)
                        ? load_index[ADDR_WIDTH-1:0] : '0;

  assign inv_load = (state == LOAD_INV) &&
                    (load_index >= C_RAM_READ_LATENCY) &&
                    (load_index <= N);
  assign inv_load_addr = load_index[ADDR_WIDTH-1:0] - C_RAM_READ_LATENCY;
  assign inv_load_data = c_ntt_dout_b;
