...
def bit_reverse(value, width):
    ...
    for index in range(width):
        if (value >> index) & 1:
            reversed_value |= 1 << (width - 1 - index)
    return reversed_value


def bit_reverse_stage(value, bits):
    reversed_value = 0
    for index in range(bits):
        if (value >> index) & 1:
            reversed_value |= 1 << (bits - 1 - index)
    return reversed_value
...
async def run_ntt(dut):
    ...
    while int(dut.done.value) == 0:
        ...
        if cycles > timeout:
            ...
            state = (
                int(dut.u_control.state.value)
                if hasattr(dut.u_control, "state")
                else -1
            )
            stage = (
                int(dut.u_control.stage.value)
                if hasattr(dut.u_control, "stage")
                else -1
            )
            butterfly = (
                int(dut.u_control.butterfly.value)
                if hasattr(dut.u_control, "butterfly")
                else -1
            )
            cycle = (
                int(dut.u_control.cycle.value)
                if hasattr(dut.u_control, "cycle")
                else -1
            )
            busy = int(dut.busy.value)
            done = int(dut.done.value)
            ...
            dut._log.error(
                f"    FSM state={state}, stage={stage}, butterfly={butterfly}, cycle={cycle}"
            )
        ...
        if cycles % 1000 == 0:
            if hasattr(dut.u_control, "stage"):
                ...
                dut._log.info(
                    f"  Cycle {cycles}: state={state}, stage={stage}, butterfly={butterfly}, cycle={cycle}"
                )
            else:
...
async def test_ntt_simple_vector(dut):
    ...
    for i in range(N):
        if results[i] != expected[i]:
            if mismatches < 10:  # Limit error output
                dut._log.error(
                    f"  Mismatch at {i}: got {results[i]}, expected {expected[i]}"
                )
            mismatches += 1
...
async def test_ntt_clock(dut):
    ...
    assert mismatches == 0, (
        f"Hardware output doesn't match Python reference ({mismatches} mismatches)"
    )
...
async def test_ntt_random_polynomials(dut):
    ...
    for test_num in range(num_tests):
        ...
        for i in range(N):
            if results[i] != expected[i]:
                if mismatches < 5:  # Limit error output
                    dut._log.error(
                        f"    Index {i}: got {results[i]}, expected {expected[i]}"
                    )
                mismatches += 1
        ...
        assert mismatches == 0, (
            f"Random test {test_num + 1} failed with {mismatches} mismatches"
        )
