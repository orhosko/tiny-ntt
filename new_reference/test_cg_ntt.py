...
def test_cg_ntt_identity():
    random.seed(0)
    a = [random.randrange(Q) for _ in range(N)]
    transformed = cg_ntt(a, OMEGA_N, Q, verbose=True)
    recovered = cg_intt(transformed, OMEGA_N, Q)
    log_vector("identity input", a)
    log_vector("identity transformed", transformed)
    log_vector("identity recovered", recovered)
    assert_equal_with_logs([x % Q for x in a], recovered, "cg_ntt_identity")


def test_nwc_poly_mult_basic():
    a = [0] * N
    b = [0] * N
    a[0] = 1
    a[1] = 2
    ...
    actual = nwc_poly_mult(a, b, PSI_2N)
    log_vector("basic a", a)
    log_vector("basic b", b)
    log_vector("basic expected", expected)
    log_vector("basic actual", actual)
    assert_equal_with_logs(expected, actual, "nwc_poly_mult_basic")


def test_nwc_poly_mult_small_example():
    a = [0] * N
    b = [0] * N
    a[0] = 1
    a[1] = 2
    ...
    assert_equal_with_logs(expected, actual, "nwc_poly_mult_small_example")


def test_nwc_poly_mult_random():
    random.seed(1)
    a = [random.randrange(Q) for _ in range(N)]
    ...
    log_vector("random expected", expected)
    log_vector("random actual", actual)
    assert_equal_with_logs(expected, actual, "nwc_poly_mult_random")


if __name__ == "__main__":
    test_cg_ntt_identity()
    test_nwc_poly_mult_basic()
    test_nwc_poly_mult_random()
    print("All CG NTT tests passed.")
